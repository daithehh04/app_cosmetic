...
import { useGetAllCategoryProductQuery } from "@/stores/slices/api/category-product.slices.api";
import {
  Table,
  TableHeader,
  TableColumn,
...
function TableProduct() {
  ...
  const { data: productCategory, isLoading: isLoadingCategory } =
    useGetAllCategoryProductQuery({ page: 1, limit: 100 });
  ...
  const categoryProduct = productCategory?.data?.categoryProduct?.map(
    (item) => {
      ...
      case "view":
        return (
          <>
            ...
            <p>Loại sản phẩm: {productDetail?.categoryProduct?.name}</p>
        ...
        return (
          <>
            ...
            <Select
              ...
              label="Loại sản phẩm"
              defaultSelectedKeys={[productDetail.categoryProduct.id]}
              className="max-max-w-full"
              onChange={(e) =>
                handleChangeProduct("category_product_id", e.target.value)
              }
            >
